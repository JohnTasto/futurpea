...
musicToMEvents c@MContext{mcTime = t, mcDur = dt} (m1 :+: m2)               = (evs1 ++ evs2, d1+d2) where
  (evs1, d1) = musicToMEvents c m1
  (evs2, d2) = musicToMEvents c {mcTime = t+d1} m2
musicToMEvents c@MContext{mcTime = t, mcDur = dt} (m1 :=: m2)               = (merge evs1 evs2, max d1 d2) where
  (evs1, d1) = musicToMEvents c m1
...
noteToMEvent c@(MContext ct ci cdur cvol) d (p, nas) = foldr nasFun e0 nas where
  e0 = MEvent{eTime = ct, ePitch = absPitch p, eInst = ci, eDur = d*cdur, eVol = cvol, eParams = []}
...
phraseToMEvents c@MContext{mcTime=t, mcInst=i, mcDur=dt} (pa:pas) m = case pa of
